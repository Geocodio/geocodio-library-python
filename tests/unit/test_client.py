...
def test_client_initialization_no_key(monkeypatch):
    ...
    with pytest.raises(
        AuthenticationError, match="No API key supplied and GEOCODIO_API_KEY is not set"
    ):
        Geocodio()
...
def test_geocode_batch_with_custom_keys(mock_request):
    ...
    assert response.results[0].formatted_address == "1109 N Highland St, Arlington, VA"
    assert (
...
def test_geocode_with_congressional_districts(mock_request):
    ...
    assert (
        response.results[0].fields.congressional_districts[0].name
        == "Virginia's 8th congressional district"
    )
    assert response.results[0].fields.congressional_districts[0].district_number == 8
    assert (
        response.results[0].fields.congressional_districts[0].congress_number == "118"
    )
