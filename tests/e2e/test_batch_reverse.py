...
def test_batch_reverse_geocoding(client):
    ...
    assert (
        "Washington" in white_house.formatted_address
        or "DC" in white_house.formatted_address
    )

    # Check third result (Apple Park)
    apple_park = response.results[2]
    assert (
        "Cupertino" in apple_park.formatted_address
        or "CA" in apple_park.formatted_address
    )
...
def test_batch_reverse_with_strings(client):
    ...
    assert (
        "Pennsylvania" in response.results[1].formatted_address
